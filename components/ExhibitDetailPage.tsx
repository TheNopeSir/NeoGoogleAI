...
const ExhibitDetailPage: React.FC<ExhibitDetailPageProps> = ({
  ...
  const renderCommentNode = (c: Comment, depth = 0) => {
      ...
      return (
          <div key={c.id} className={`flex flex-col ${depth > 0 ? 'ml-4 md:ml-8 pl-4 mt-2 border-l-2 ' + (isDark ? 'border-white/10' : 'border-black/10') : 'mt-4'}`}>
              <div 
                id={`comment-${c.id}`} 
                className={`p-3 border transition-all ${isWinamp ? 'bg-black border-[#505050]' : isDark ? 'rounded-xl bg-white/5 border-white/5 hover:border-white/10' : 'rounded-xl bg-white border-black/5 shadow-sm'}`}
              >
                  <div className="flex justify-between items-start mb-1">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => handleReply(c)} className="text-gray-500 hover:text-current transition-colors" title="Ответить">
  ...
  return (
    <div className={`w-full min-h-full pb-20 animate-in slide-in-from-right-8 fade-in duration-500 ${isWinamp ? 'font-mono text-gray-300' : theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
        ...
        <div className={`flex items-center justify-between mb-4 border-b pb-2 ${isDark ? 'border-white/5' : 'border-black/5'}`}>
            <div className="flex items-center gap-2 text-[10px] font-mono opacity-50 uppercase">
                ...
                <span className="hover:text-current cursor-pointer">{exhibit.category}</span>
            </div>

            <div className="flex items-center gap-3">
                {(isOwner || isAdmin) && (
                    <div className="flex gap-2">
                        {onEdit && <button onClick={() => onEdit(exhibit)} className="text-purple-400 hover:text-current transition-all"><Edit2 size={16}/></button>}
                        {onDelete && <button onClick={() => onDelete(exhibit.id)} className="text-red-500 hover:text-current transition-all"><Trash2 size={16}/></button>}
                    </div>
                )}
                <div className={`h-4 w-[1px] ${isDark ? 'bg-white/10' : 'bg-black/10'}`}></div>
                <div className="relative">
                    <button onClick={() => setShowShareMenu(!showShareMenu)} className={`hover:text-current transition-all ${shareCopied ? 'text-green-500' : 'opacity-60'}`}><Share2 size={16}/></button>
                    {showShareMenu && (
                        <div className={`absolute right-0 top-6 w-40 border rounded-xl shadow-2xl z-50 p-1 animate-in slide-in-from-top-2 ${isDark ? 'bg-dark-surface border-white/10' : 'bg-white border-black/10'}`}>
                            <button onClick={() => handleShare('tg')} className={`w-full text-left p-2 rounded text-[10px] font-bold flex items-center gap-2 ${isDark ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}><Send size={12} className="text-blue-400"/> TELEGRAM</button>
                            <button onClick={() => handleShare('wa')} className={`w-full text-left p-2 rounded text-[10px] font-bold flex items-center gap-2 ${isDark ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}><MessageCircle size={12} className="text-green-500"/> WHATSAPP</button>
                            <button onClick={() => handleShare('copy')} className={`w-full text-left p-2 rounded text-[10px] font-bold flex items-center gap-2 ${isDark ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}><Share2 size={12}/> COPY LINK</button>
                        </div>
                    )}
                </div>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
            ...
            <div className="w-full lg:w-[45%] space-y-4 lg:sticky lg:top-20">
                <div 
                    className={`relative aspect-square w-full overflow-hidden border transition-all duration-500 group ${isWinamp ? 'bg-black border-[#505050]' : (isDark ? 'rounded-2xl border-white/10 bg-black' : 'rounded-2xl border-black/10 bg-gray-100')} ${isCursed ? 'shadow-[0_0_30px_red]' : ''}`}
            ...
            <div className="flex-1 w-full">
                <div className={`p-5 md:p-6 border mb-4 ${isWinamp ? 'bg-[#191919] border-[#505050]' : (isDark ? 'bg-dark-surface border-white/10 rounded-2xl' : 'bg-white border-black/10 shadow-lg rounded-2xl')}`}>
                    
                    {/* Header Info */}
                    <div className="flex flex-col gap-4 mb-6">
                        <div className="flex flex-wrap gap-2">
                            <span className={`px-2 py-0.5 text-[9px] font-pixel font-bold uppercase border rounded ${isWinamp ? 'border-[#00ff00] text-[#00ff00]' : 'border-green-500/50 text-green-500 bg-green-500/10'}`}>{exhibit.category}</span>
                            <span className={`px-2 py-0.5 text-[9px] font-bold font-pixel border rounded flex items-center gap-1 uppercase ${isWinamp ? 'border-[#00ff00] text-[#00ff00]' : `${tier.bgColor} ${tier.color} ${isDark ? 'border-white/5' : 'border-black/5'}`}`}><TierIcon size={10} /> {tier.name}</span>
                            {tradeStatus !== 'NONE' && (
                        ...
                        <h1 className={`text-xl md:text-3xl font-bold font-pixel leading-tight ${isCursed ? 'text-red-500 italic' : (isWinamp ? 'text-[#00ff00]' : '')}`}>{exhibit.title}</h1>
                        
                        {/* Compact Stats Toolbar */}
                        <div className={`flex items-center gap-4 text-xs font-mono opacity-70 border-b pb-4 ${isDark ? 'border-white/5' : 'border-black/5'}`}>
                    ...
                    {/* Author Row */}
                    <div className={`flex items-center justify-between mb-6 pb-4 border-b ${isDark ? 'border-white/5' : 'border-black/5'}`}>
                        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onAuthorClick(exhibit.owner)}>
                            <img src={getUserAvatar(exhibit.owner)} className={`w-10 h-10 rounded-full border ${isDark ? 'border-white/20' : 'border-black/10'}`} />
                            <div>
                        ...
                        {!isOwner && ( 
                            <button onClick={() => onFollow(exhibit.owner)} className={`px-3 py-1.5 text-[9px] font-bold font-pixel border rounded transition-all ${isFollowing ? 'border-current opacity-40' : (isDark ? 'bg-white/10 hover:bg-white/20 border-transparent' : 'bg-black/5 hover:bg-black/10 border-transparent')}`}>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-1">
                                {nonEmptySpecs.map(([key, val]) => (
                                    <div key={key} className={`px-2 py-1.5 border rounded flex flex-col justify-center ${isWinamp ? 'bg-black border-[#505050]' : isDark ? 'bg-black/20 border-white/5' : 'bg-gray-50 border-black/5'}`}>
                                ...
                                {exhibit.condition && (
                                    <div className={`px-2 py-1.5 border rounded flex flex-col justify-center ${isWinamp ? 'bg-black border-[#505050]' : isDark ? 'bg-black/20 border-white/5' : 'bg-gray-50 border-black/5'}`}>
                                        <div className="text-[7px] uppercase opacity-40 font-pixel tracking-wider mb-0.5">СОСТОЯНИЕ</div>
                                        <div className="font-bold font-mono text-[10px] text-green-500 uppercase leading-tight">{exhibit.condition}</div>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {linkedArtifacts.length > 0 && (
                        <div className={`mt-6 pt-4 border-t ${isDark ? 'border-white/5' : 'border-black/5'}`}>
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                                {linkedArtifacts.map(link => (
                                    <div key={link.id} onClick={() => onExhibitClick(link)} className="flex-shrink-0 w-20 group cursor-pointer">
                                        <div className={`aspect-square rounded-lg overflow-hidden border relative ${isDark ? 'border-white/10 bg-black/20' : 'border-black/10 bg-gray-100'}`}>
                                            <img src={link.imageUrls[0]} className="w-full h-full object-cover transition-transform group-hover:scale-110" draggable="false" />
                                        </div>
                ...
                {/* Comments Section */}
                <div className={`p-5 rounded-2xl border ${isWinamp ? 'bg-[#191919] border-[#505050]' : isDark ? 'bg-dark-surface border-white/5' : 'bg-white border-black/5'}`}>
                    ...
                    <div className="space-y-4 mb-6 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                        {comments.length === 0 ? ( <div className={`text-center py-8 opacity-30 text-[10px] font-pixel uppercase tracking-widest border border-dashed rounded-xl ${isDark ? 'border-white/10' : 'border-black/10'}`}>ТИШИНА В ЭФИРЕ</div> ) : ( 
                    ...
                    <div className="flex flex-col gap-2 relative">
                        {mentionQuery !== null && filteredUsers.length > 0 && (
                            <div className={`absolute bottom-full mb-2 left-0 w-64 border rounded-xl overflow-hidden shadow-2xl z-50 ${isDark ? 'bg-black border-white/10' : 'bg-white border-black/10'}`}>
                                {filteredUsers.map(u => (
                                    <button 
                                        key={u.username}
                                        onClick={() => selectMention(u.username)}
                                        className={`w-full flex items-center gap-2 p-2 text-left transition-colors ${isDark ? 'hover:bg-white/10' : 'hover:bg-gray-100'}`}
                                    >
                        ...
                        {replyTo && (
                            <div className={`flex items-center justify-between text-[10px] font-mono p-2 rounded-lg border ${isDark ? 'bg-white/5 border-white/5' : 'bg-black/5 border-black/5'}`}>
                        ...
                        <div className="flex gap-2">
                            <input 
                                ...
                                className={`flex-1 border px-3 py-2.5 font-mono text-xs focus:outline-none focus:border-green-500 transition-colors rounded-lg ${isDark ? 'bg-black/40 border-white/10' : 'bg-white border-black/10'}`} 
                                onKeyDown={(e) => { 
